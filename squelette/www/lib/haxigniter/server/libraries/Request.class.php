...
class haxigniter_server_libraries_Request {
	...
	public $config;
	public function createController($controllerName, $controllerArgs) {
		$controllerClass = haxigniter_server_libraries_Request_0($this, $controllerArgs, $controllerName);
	...
	public function internal($request, $method, $getPostData, $requestData) {
		if($method === null) {
			$method = "GET";
		}
	...
	public function execute($url, $method, $getPostData, $requestContent, $outputFunction) {
		if($method === null) {
			$method = "GET";
		}
		$parsedUrl = haxigniter_server_libraries_Request_2($this, $getPostData, $method, $outputFunction, $requestContent, $url);
		$controller = $this->config->router->createController($this->config, $parsedUrl);
		$requestData = null;
...
function haxigniter_server_libraries_Request_0(&$»this, &$controllerArgs, &$controllerName) {
...
function haxigniter_server_libraries_Request_2(&$»this, &$getPostData, &$method, &$outputFunction, &$requestContent, &$url) {
	{
		$parsedUrl = new haxigniter_common_libraries_ParsedUrl($url);
		_hx_deref(new haxigniter_server_libraries_Url($»this->config))->testValidUri($parsedUrl->path);
		return $parsedUrl;
