...
class microbe_factoryType_CollectionBehaviour implements microbe_factoryType_IBehaviour{
	...
	public function create($voName, $element, $field, $formulaire) {
		...
		if($this->data->id !== null) {
			haxe_Log::trace("y'a de la data" . $this->data->id . "voName" . $voName, _hx_anonymous(array("fileName" => "CollectionBehaviour.hx", "lineNumber" => 75, "className" => "microbe.factoryType.CollectionBehaviour", "methodName" => "create")));
	...
	public function record($source, $data) {
		haxe_Log::trace("record collection" . $data->id, _hx_anonymous(array("fileName" => "CollectionBehaviour.hx", "lineNumber" => 289, "className" => "microbe.factoryType.CollectionBehaviour", "methodName" => "record")));
		...
		if($data->id === null) {
			...
			while($»it->hasNext()) {
				$a = $»it->next();
				haxe_Log::trace("bip=" . $a->type, _hx_anonymous(array("fileName" => "CollectionBehaviour.hx", "lineNumber" => 306, "className" => "microbe.factoryType.CollectionBehaviour", "methodName" => "record")));
				...
				$parser->data = $data1;
				$child = $parser->record();
				haxe_Log::trace("child_titre=" . Reflect::field($child, "titre"), _hx_anonymous(array("fileName" => "CollectionBehaviour.hx", "lineNumber" => 313, "className" => "microbe.factoryType.CollectionBehaviour", "methodName" => "record")));
			...
			haxe_Log::trace("------hopopop=" . $refs, _hx_anonymous(array("fileName" => "CollectionBehaviour.hx", "lineNumber" => 324, "className" => "microbe.factoryType.CollectionBehaviour", "methodName" => "record")));
	...
	static function creeEmptyCollection($voName, $element, $field, $graine) {
		...
		while($»it->hasNext()) {
			...
			$item->type = $item->type;
			$item->voName = $item->voName;
			$item->elementId = $collec->elementId . "_" . $item->field . "_" . $graine;
			$item->field = $item->field;
			$elem = microbe_factoryType_CollectionBehaviour::creeAjaxFormElement($item, Std::string($graine));
			$micros->add($elem);
			unset($elem);
	...
	static function creeAjaxFormElement($microfield, $graine) {
		if($graine === null) {
			$graine = "";
		}
