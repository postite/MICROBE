...
class microbe_Api {
	...
	public function recClassMap() {
		haxe_Log::trace("record" . _hx_string_rec($this->map->id, ""), _hx_anonymous(array("fileName" => "Api.hx", "lineNumber" => 342, "className" => "microbe.Api", "methodName" => "recClassMap")));
		$voInstance = null;
		if($this->map->id !== null) {
			haxe_Log::trace("map.id!=null", _hx_anonymous(array("fileName" => "Api.hx", "lineNumber" => 345, "className" => "microbe.Api", "methodName" => "recClassMap")));
			...
			haxe_Log::trace("DBINFOS" . Std::string($dbInfos), _hx_anonymous(array("fileName" => "Api.hx", "lineNumber" => 352, "className" => "microbe.Api", "methodName" => "recClassMap")));
				...
				while($_g < $_g1->length) {
					...
					if($index->unique) {
						...
						while($_g2 < $_g3->length) {
							$key = $_g3[$_g2];
							++$_g2;
							haxe_Log::trace("unique=" . $key, _hx_anonymous(array("fileName" => "Api.hx", "lineNumber" => 357, "className" => "microbe.Api", "methodName" => "recClassMap")));
							if(null == $this->map->fields) throw new HException('null iterable');
							$»it = $this->map->fields->iterator();
							while($»it->hasNext()) {
								$f = $»it->next();
								if($f->field === $key) {
									haxe_Log::trace("found unique KEY", _hx_anonymous(array("fileName" => "Api.hx", "lineNumber" => 361, "className" => "microbe.Api", "methodName" => "recClassMap")));
									$value = addslashes($f->value);
									$pip = $manager->unsafeExecute("Select * from " . $dbInfos->name . " where " . $key . "='" . $value . "'");
									haxe_Log::trace("pip=" . _hx_string_rec($pip->getLength(), ""), _hx_anonymous(array("fileName" => "Api.hx", "lineNumber" => 364, "className" => "microbe.Api", "methodName" => "recClassMap")));
									if($pip->getLength() > 0) {
		...
		haxe_Log::trace("after", _hx_anonymous(array("fileName" => "Api.hx", "lineNumber" => 383, "className" => "microbe.Api", "methodName" => "recClassMap")));
		...
		haxe_Log::trace("fullSpod" . Std::string($fullSpod), _hx_anonymous(array("fileName" => "Api.hx", "lineNumber" => 403, "className" => "microbe.Api", "methodName" => "recClassMap")));
		return $fullSpod;
	}
