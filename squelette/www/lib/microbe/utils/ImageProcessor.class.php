...
class microbe_utils_ImageProcessor {
	...
	public function getOutput($flush = null) {
		if($flush === null) {
			$flush = false;
		}
	...
	public function getCacheName() {
		$hash = $this->getHash();
		$ext = microbe_utils_ImageProcessor_0($this, $hash);
		return $hash . "." . $ext;
	}
	...
	public function getHash() {
		...
		while($»it->hasNext()) {
			...
			while($_g < $_g1->length) {
				$field = $_g1[$_g];
				++$_g;
				$s .= "&" . $field . "=" . Std::string(Reflect::field($action, $field));
				unset($field);
			}
			unset($_g1,$_g);
		}
		$stat = sys_FileSystem::stat($this->fileName);
		$s .= "&" . Std::string($stat->mtime) . "&" . Std::string($stat->ctime) . "&" . microbe_utils_ImageProcessor::$revision . _hx_string_rec($this->quality, "") . _hx_string_rec(((($this->saveAlpha) ? 1 : 0)), "");
	...
	public function getFileFormat($file) {
		...
		return microbe_utils_ImageProcessor_1($this, $ext, $file);
	}
	...
	public function applyRotation($angle, $CW = null, $bgcolor = null, $transparentBg = null) {
	...
	public function queueRotate($CW = null) {
		if($CW === null) {
			$CW = true;
		}
	...
	public function init() {
		$this->resource = microbe_utils_ImageProcessor_3($this);
		$this->mimeType = microbe_utils_ImageProcessor_4($this);
...
function microbe_utils_ImageProcessor_0(&$»this, &$hash) {
	$»t = ($»this->format);
	switch($»t->index) {
	case 2:
	{
...
function microbe_utils_ImageProcessor_1(&$»this, &$ext, &$file) {
	switch($ext) {
	case "jpg":case "jpeg":{
...
function microbe_utils_ImageProcessor_3(&$»this) {
	$»t = ($»this->outputFormat);
	switch($»t->index) {
	case 0:
	{
...
function microbe_utils_ImageProcessor_4(&$»this) {
	$»t = ($»this->outputFormat);
	switch($»t->index) {
	case 0:
	{
