...
class microbe_TagManager {
	...
	static function getTaxos($spod) {
		...
		haxe_Log::trace("spodTAble=" . $spodTable, _hx_anonymous(array("fileName" => "TagManager.hx", "lineNumber" => 88, "className" => "microbe.TagManager", "methodName" => "getTaxos")));
		...
		haxe_Log::trace("resultSet=" . Std::string($resultSet) . "spodTAble=" . $spodTable, _hx_anonymous(array("fileName" => "TagManager.hx", "lineNumber" => 96, "className" => "microbe.TagManager", "methodName" => "getTaxos")));
	...
	static function getTagsById($spod, $spodId) {
		haxe_Log::trace("getTags", _hx_anonymous(array("fileName" => "TagManager.hx", "lineNumber" => 116, "className" => "microbe.TagManager", "methodName" => "getTagsById")));
		$liste = microbe_TagManager::getTaxoBySpodID($spod, $spodId);
		$tags = new HList();
		if(null == $liste) throw new HException('null iterable');
		$»it = $liste->iterator();
	...
	static function maptoSpod($res) {
		...
		while($»it->hasNext()) {
			$key = $»it->next();
			haxe_Log::trace("key=" . $key, _hx_anonymous(array("fileName" => "TagManager.hx", "lineNumber" => 262, "className" => "microbe.TagManager", "methodName" => "maptoSpod")));
			$spod->{$key} = Reflect::field($res, $key);
		}
		return $spod;
	}
	static function getSpodTable($spod) {
		haxe_Log::trace("getSpoTable" . $spod, _hx_anonymous(array("fileName" => "TagManager.hx", "lineNumber" => 272, "className" => "microbe.TagManager", "methodName" => "getSpodTable")));
		$voPackage = "vo.";
		$cap = microbe_TagManager::firstUpperCase($spod);
		$spodable = Type::resolveClass($voPackage . $cap);
		haxe_Log::trace("spodable" . Std::string($spodable), _hx_anonymous(array("fileName" => "TagManager.hx", "lineNumber" => 277, "className" => "microbe.TagManager", "methodName" => "getSpodTable")));
		$manager = Reflect::field($spodable, "manager");
		haxe_Log::trace("manager" . Std::string($manager), _hx_anonymous(array("fileName" => "TagManager.hx", "lineNumber" => 279, "className" => "microbe.TagManager", "methodName" => "getSpodTable")));
