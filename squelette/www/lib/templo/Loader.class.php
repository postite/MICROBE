...
class templo_Loader {
		...
		if(templo_Loader::$MACROS !== null && templo_Loader::$MACROS !== "") {
			...
			}
			{
				...
				while($_g < $macrosfiles->length) {
					$mf = $macrosfiles[$_g];
					++$_g;
					require_once($this->tmpFileId($mf));
					unset($mf);
				}
			}
	...
	public function tmpFileId($path) {
		...
		return templo_Loader::$TMP_DIR . $path . ".php";
	}
	...
	public function compileTemplate($path) {
		...
		if(file_exists($tmpFile)) {
			$macroStamp = ((templo_Loader::$MACROS !== null && file_exists(templo_Loader::$BASE_DIR . templo_Loader::$MACROS)) ? sys_FileSystem::stat(templo_Loader::$BASE_DIR . templo_Loader::$MACROS)->mtime->getTime() : null);
			$sourceStamp = sys_FileSystem::stat(templo_Loader::$BASE_DIR . $path)->mtime->getTime();
