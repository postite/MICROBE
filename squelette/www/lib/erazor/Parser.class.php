...
class erazor_Parser {
	...
	public function parseCodeBlock($template) {
		if($this->bracketStack->length > 0 && $this->peek($template, null) === "}") {
			...
			if(erazor_Parser_0($this, $template)) {
				throw new HException(erazor_Parser::$bracketMismatch);
			}
		...
		if($startBrace === "{") {
			return _hx_anonymous(array("block" => erazor_TBlock::codeBlock($noBraces), "length" => strlen($str) + 1));
		} else {
			return _hx_anonymous(array("block" => erazor_TBlock::printBlock($noBraces), "length" => strlen($str) + 1));
		}
	}
	...
	public function parseVariable($template) {
		...
		return _hx_anonymous(array("block" => erazor_TBlock::printBlock($output), "length" => strlen($output) + 1));
	}
	...
	public function parseConditional($template) {
		$str = $this->parseScriptPart($template, "", "{");
		return _hx_anonymous(array("block" => erazor_TBlock::codeBlock(_hx_substr($str, 1, null)), "length" => strlen($str)));
	}
	...
	public function acceptIdentifier($template) {
		$first = true;
		$self = $this;
		return $this->accept($template, array(new _hx_lambda(array(&$first, &$self, &$template), "erazor_Parser_1"), 'execute'), false);
	}
	public function isIdentifier($char, $first = null) {
		if($first === null) {
			$first = true;
		}
		return erazor_Parser_2($this, $char, $first);
	}
	public function accept($template, $acceptor, $throwAtEnd) {
		return $this->parseString($template, array(new _hx_lambda(array(&$acceptor, &$template, &$throwAtEnd), "erazor_Parser_3"), 'execute'), $throwAtEnd);
	}
...
function erazor_Parser_0(&$»this, &$template) {
...
function erazor_Parser_1(&$first, &$self, &$template, $chr) {
	{
...
function erazor_Parser_2(&$»this, &$char, &$first) {
	if($first) {
...
function erazor_Parser_3(&$acceptor, &$template, &$throwAtEnd, $chr) {
	{
