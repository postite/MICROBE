...
class hscript_Parser {
				...
				while(true) {
					...
					case 120:{
						...
						while(true) {
							...
							default:{
								$this->char = $char;
								$v = hscript_Parser_3($this, $char, $exp, $n, $n1);
	...
	public function parseStructure($id) {
		return hscript_Parser_6($this, $id);
	}
	public function makeBinop($op, $e1, $e) {
		return hscript_Parser_7($this, $e, $e1, $op);
	}
	public function makeUnop($op, $e) {
		return hscript_Parser_8($this, $e, $op);
	}
	...
	public function isBlock($e) {
		return hscript_Parser_9($this, $e);
	}
	...
	public function parse($s) {
		...
		return hscript_Parser_10($this, $a, $s);
	}
...
function hscript_Parser_0(&$퍁his, &$t) {
	$퍁 = ($t);
	switch($퍁->index) {
	case 0:
	{
...
function hscript_Parser_1(&$퍁his, &$c) {
	$퍁 = ($c);
	switch($퍁->index) {
	case 0:
	$v = $퍁->params[0];
...
function hscript_Parser_2(&$퍁his) {
	{
		$_this = $퍁his->tokens;
		$k = $_this->head;
		if($k === null) {
...
function hscript_Parser_3(&$퍁his, &$char, &$exp, &$n, &$n1) {
	try {
		return hscript_Const::CInt(hscript_Parser_11($char, $exp, $n, $n1));
	}catch(Exception $팫) {
		$_ex_ = ($팫 instanceof HException) ? $팫->e : $팫;
		$e = $_ex_;
		{
...
function hscript_Parser_4(&$퍁his) {
	try {
...
function hscript_Parser_6(&$퍁his, &$id) {
	switch($id) {
	case "if":{
		$cond = $퍁his->parseExpr();
		$e1 = $퍁his->parseExpr();
	...
	case "try":{
		...
		$tk = $퍁his->token();
		$vname = hscript_Parser_12($e, $id, $tk);
		{
			$t = $퍁his->token();
			if($t !== hscript_Token::$TDoubleDot) {
				$퍁his->unexpected($t);
...
function hscript_Parser_7(&$퍁his, &$e, &$e1, &$op) {
	$퍁 = ($e);
	switch($퍁->index) {
	case 6:
...
function hscript_Parser_8(&$퍁his, &$e, &$op) {
	$퍁 = ($e);
	switch($퍁->index) {
	case 6:
...
function hscript_Parser_9(&$퍁his, &$e) {
	$퍁 = ($e);
	switch($퍁->index) {
	case 4:
	case 21:
...
function hscript_Parser_10(&$퍁his, &$a, &$s) {
	if($a->length === 1) {
		return $a[0];
	} else {
		return hscript_Expr::EBlock($a);
...
function hscript_Parser_11(&$char, &$exp, &$n, &$n1) {
	{
		if(($n1 >> 30 & 1) !== _hx_shift_right($n1, 31)) {
			throw new HException("Overflow " . Std::string($n1));
		}
		return $n1 & -1;
	}
}
function hscript_Parser_12(&$e, &$id, &$tk) {
	$퍁 = ($tk);
	switch($퍁->index) {
	case 2:
	$id1 = $퍁->params[0];
